...
const tools = [
  {
    ...
    description:
      "Propose one safe spreadsheet operation based on the user's request.",
    input_schema: {
      type: "object",
...
export async function proposeOperation(
  ...
  const toolUse = response.content.find(
    (
      ...
      block.type === "tool_use" &&
      block.name === "propose_spreadsheet_operation",
  );

  if (!toolUse) {
    throw new Error(
      "Claude did not return a spreadsheet operation.",
    );
  }
...
function validateProposal(
  ...
  const deletionOperations: OperationName[] = [
    "delete_records",
    "delete_column",
  ];

  const isMutation = mutationOperations.includes(
    proposal.operation,
  );

  const isDeletion = deletionOperations.includes(
    proposal.operation,
  );
  ...
  if (
    ...
    throw new Error(
      `Unknown target column: ${proposal.targetColumn}`,
    );
  }

  if (
    isMutation &&
    !request.permissions.includes("write")
  ) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      "The user does not have delete permission.",
    );
  }
  ...
  if (
    proposal.estimatedAffectedRows > 100 &&
    !proposal.requiresConfirmation
  ) {
    throw new Error(
      "Large operations must require confirmation.",
    );
  }

  if (
    proposal.operation === "set_formula" &&
    !proposal.formula
  ) {
    throw new Error(
      "A formula operation requires a formula.",
    );
  }

  if (
    proposal.operation === "clarification_required" &&
    !proposal.clarificationQuestion
  ) {
    throw new Error(
      "A clarification request requires a question.",
    );
  }
}
```
...
```json
{
  ...
  "clarificationQuestion":
    "How much should salary increase, and which records should be updated?",
  "explanation":
    "The increase amount and affected employees were not specified.",
