...
**Note**: before the event bubbling phase, there is the [event capturing](../describe-event-capturing/main) phase, which is the opposite of bubbling, where the event travels down from the document root to the target element.
...
Event bubbling is the basis for a technique called [event delegation](../explain-event-delegation/main), where you attach a single event handler to a common ancestor of multiple elements to handle events for those elements efficiently. This is particularly useful when you have a large number of similar elements, like a list of items, and you want to avoid attaching individual event handlers to each item.
...
_Source: [greatfrontend/top-javascript-interview-questions](https://github.com/greatfrontend/top-javascript-interview-questions/blob/main/questions/describe-event-bubbling/en-US.mdx)_
