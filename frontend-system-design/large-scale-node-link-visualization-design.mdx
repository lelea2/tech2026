...
![Large-scale node-link visualization architecture](./large-scale-node-link-visualization-architecture.svg)
...
function upsertEdge(store: GraphStore, edge: GraphEdge) {
  ...
  if (
    previous &&
    previous.entityVersion >= edge.entityVersion
  ) {
    return;
  }
  ...
  addToSet(
    store.outgoingEdgeIdsByNode,
    edge.sourceId,
    edge.id
  );

  addToSet(
    store.incomingEdgeIdsByNode,
    edge.targetId,
    edge.id
  );
}
